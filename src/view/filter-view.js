import AbstractRadioListView from './radio-list-view.js';

const createFilterTemplate = (filter) => {
  const {type, isDisabled, isChecked} = filter;
  return `
<div class="trip-filters__filter">
  <input data-item = "${type}" id="filter-${type}" class="trip-filters__filter-input  visually-hidden" type="radio" name="trip-filter" value="${type}" ${isChecked ? 'checked' : ''}>
  <label class="trip-filters__filter-label" for="filter-${type}" ${isDisabled ? 'disabled' : ''}>${type}</label>
</div>`;
};
